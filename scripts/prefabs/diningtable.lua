...
local function ongetitem(inst, giver, item)
    local wilttime = item.components.perishable ~= nil and item.components.perishable:GetPercent() * TUNING.ENDTABLE_FLOWER_WILTTIME or TUNING.ENDTABLE_FLOWER_WILTTIME  GiveFlower(inst, GetRandomItem(FLOWER_MAP[item.prefab].foodnames), wilttime, giver)   
    inst.SoundEmitter:PlaySound("dontstarve/creatures/together/stagehand/hit")  
    inst.AnimState:PlayAnimation("hit")  
    inst.AnimState:PushAnimation("idle") 
end  
	...
	if data ~= nil then  
		if data.foodname ~= nil then  
			...
			inst.AnimState:ShowSymbol("swap_flower")  
				...
				else  inst.AnimState:OverrideSymbol("swap_flower", "cook_pot_food", product)  
			end  
		end  
	end 
end  
...
local function ongivenitem(inst, giver, item)  
 local count = 1  
	...
	if inst.foodname == nil then
		...
		else  inst.foodnum = inst.foodnum + count  
	end 
end  
